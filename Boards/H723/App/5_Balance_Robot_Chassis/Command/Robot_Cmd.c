...
void Robot_Cmd_Init(void)
{
    ...
    chassis_cmd.target_length=0.35f;
}

void Robot_Cmd_Update(void)
{
    System_State_Report_Remote(DBUS.offline.is_online);//向系统状态模块传入遥控器在线状态 DBUS.offline.is_online
...
static void Cmd_Update_Remote_Ctrl(void)
{
    ...
    chassis_cmd.target_vw = -(float)DBUS.Remote.CH0 * RC_ROCKER_VW_COEF ;//+ (float)VT13.Remote.Channel[3] * RC_ROCKER_VW_COEF;
    chassis_cmd.target_length += (float)DBUS.Remote.CH3 * RC_LENGTH_COEF;
    chassis_cmd.target_roll = -(float)DBUS.Remote.CH2 * RC_ROLL_COEF ;
